...
def list_questions_admin(_: User = Depends(require_manager), db: Session = Depends(get_db)):
...
def create_question(data: VotingQuestionCreate, admin: User = Depends(require_manager), db: Session = Depends(get_db)):
...
def update_question(question_id: int, data: VotingQuestionUpdate, admin: User = Depends(require_manager), db: Session = Depends(get_db)):
...
def delete_question(question_id: int, admin: User = Depends(require_manager), db: Session = Depends(get_db)):
    q = db.query(VotingQuestion).filter(VotingQuestion.id == question_id).first()
    if not q:
        raise HTTPException(status_code=404, detail="Question not found.")
    db.delete(q)
...
def add_option(question_id: int, data: VotingOptionCreate, _: User = Depends(require_manager), db: Session = Depends(get_db)):
    q = db.query(VotingQuestion).filter(VotingQuestion.id == question_id).first()
    if not q:
        raise HTTPException(status_code=404, detail="Question not found.")
...
def update_option(option_id: int, data: VotingOptionUpdate, _: User = Depends(require_manager), db: Session = Depends(get_db)):
...
def delete_option(option_id: int, _: User = Depends(require_manager), db: Session = Depends(get_db)):
    option = db.query(VotingOption).filter(VotingOption.id == option_id).first()
    if not option:
        raise HTTPException(status_code=404, detail="Option not found.")
    db.delete(option)
...
def get_question_results(question_id: int, _: User = Depends(require_manager), db: Session = Depends(get_db)):
